...
def main() -> int:
    ...
    profile = os.getenv("PP_BROWSER_PROFILE", str(Path.home() / ".ev-bot" / "prizepicks-profile"))
    ...
    with sync_playwright() as playwright:
        context = playwright.chromium.launch_persistent_context(
            profile,
            headless=os.getenv("PP_HEADLESS", "false").lower() in {"1", "true", "yes"},
        )
        page = context.new_page()
        page.goto("https://app.prizepicks.com/", wait_until="domcontentloaded", timeout=60000)
        ...
        shot = screenshot_dir / f"{entry.get('id', 'entry')}-pp.png"
        ...
        # Submission selectors change frequently — fail closed until manually verified.
        print(json.dumps({
            "status": "failed",
